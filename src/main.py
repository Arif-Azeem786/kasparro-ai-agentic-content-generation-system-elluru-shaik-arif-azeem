...
def main():
    ...
    # if assembler added 'draft_page' and you want to persist outputs:
    run_id = final_state.get("run_id")
    draft = final_state.get("draft_page")
    faq = {"title": f"FAQ - {final_state.get('product',{}).get('Product Name', final_state.get('product',{}).get('name',''))}", "faq": final_state.get("qa_pairs", [])}
    # write outputs if not dry_run
    if not args.dry_run:
        import json
        import time
